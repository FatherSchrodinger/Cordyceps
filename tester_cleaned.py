...
def ep_azon_generálása():
    letezo_epazon = {epulet.ep_azon for epulet in epuletek_list}

    while True:
        uj_epazon = random.randint(100, 999)
        if uj_epazon not in letezo_epazon:
            return uj_epazon
        
def projekt_azon_generálása():
    letezo_pazon = {int(projekt.projekt_azon) for projekt in varosfejlesztes_list if hasattr(projekt, 'projekt_azon')}
    while True:
        uj_pazon = random.randint(100, 999)
        if uj_pazon not in letezo_pazon:
            return uj_pazon
...
def uj_epulet_epitese():
    ...
    while True:
        ...
        varosfejlesztes_list.append({
            "projekt_azon": projekt_azon,
            "nev": nev,
            "tipus": tipus,
            "koltseg": koltseg,
            "kezdes": kezdes,
...
if penzkeret > 0 and lakosok_elegedettsege > min_elegedettseg:
    for honap in range(fordulok_szama):
        ...
        for project in leendo_epuletek + javitando_epuletek:
            if project["hatralevo_honap"] > 0 and not project.get("kesz", False):
        ...
        for project in elkeszult_projektek:
            project["kesz"] = True 
            uj_epulet = Epuletek(
                project["ep_azon"], project["nev"], project["tipus"], project["epitesi_ev"], project["hasznos_terulet_m2"]
            )
        ...
        epitesi_koltseg = sum(p['havi_koltseg'] for p in leendo_epuletek if p['hatralevo_honap'] > 0 and not p.get("kesz", False))
